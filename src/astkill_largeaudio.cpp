...
int main(int argc, char *argv[])
{
	...
	if (frame_accept.get_type() != IAX2_FRAME_TYPE_IAX2 || frame_accept.get_subclass() != IAX2_SUBCLASS_ACCEPT) {
	...
	iax2_frame frame_answer = iax2_frame(pkt_buf, res);
	if (frame_answer.get_type() != IAX2_FRAME_TYPE_CONTROL || frame_answer.get_subclass() != 4) {
		printf("Did not receive ANSWER\n");
		exit(1);
	}

	frame_ack.set_in_seq_num(2).set_out_seq_num(1);
	if (frame_ack.send(&remote_addr, sockfd)) {
		printf("sending ACK failed\n");
		exit(1);
	}

	iax2_frame frame_boom;
	frame_boom.set_direction(IAX2_DIRECTION_OUT).set_shell(IAX2_FRAME_FULL). \
		set_type(IAX2_FRAME_TYPE_VOICE).set_subclass(IAX2_FORMAT_ULAW). \
		set_in_seq_num(2).set_out_seq_num(1). \
		set_source_call_num(1).set_dest_call_num(remote_call_num). \
		set_raw_data(large_audio_buf, sizeof(large_audio_buf));

	if (frame_boom.send(&remote_addr, sockfd)) {
		printf("Failed to send VOICE full frame\n");
		exit(1);
	}
